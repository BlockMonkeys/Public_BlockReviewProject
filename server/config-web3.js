...
const rpcURL = "https://rinkeby.infura.io/v3/faaa936ffa19470d8a542e62affa4795";
const paymasterAddress = "0xA6e10aA9B038c9Cddea24D2ae77eC3cE38a0c016";
const { RelayProvider } = require("@opengsn/provider");

const coinContract = {
    address : "0x9Aa7b968Ff7b3FbB952e97292e6D10C9871c94ef",
    abi: [
        {
            "inputs": [
                {
...
const nftContract = {
    address: "0x6665934c0bc7D0aEfE6A7b6c7620b1d9FBC69c10",
    abi:[
        {
            "inputs": [
                {
...
module.exports = (async function(){
    ...
    let gsnProvider, contract_review, contract_coin;
    ...
    const initialize = async() => {
        ...
        contract_review = new web3.eth.Contract(
            reviewContract.abi,
            reviewContract.address
        )
    
        contract_coin = new web3.eth.Contract(
            coinContract.abi,
            coinContract.address
        )
        console.log(`✅ GSN is Begun`);
    }
    await initialize();
    
    const send_GSNTX = async(func, pubKey, privateKey=null) => {
        const data = func.encodeABI();
        const to = reviewContract.address;
        const from = pubKey;
        ...
        if(privateKey !== null){
            // 내부월렛 사용자
            await gsnProvider.addAccount(privateKey);
        }
        
        // 외부월렛 사용자
        const result = await func.send({ from : pubKey, gasPrice: "20000000000", gas: estimatedGas });
        return result;
    }
